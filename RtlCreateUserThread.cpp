...
int main()
{
	printf("This Injection used NtCreateThreadEx\r\n");
	// 首先提权一波
...
BOOL InjectDll(UINT32 ProcessId)
{
	...
	pfnRtlCreateUserThread RtlCreateUserThread =
		(pfnRtlCreateUserThread)GetProcAddress(GetModuleHandle(L"ntdll.dll"), "RtCreateUserThread");

	HANDLE ThreadHandle = NULL;
	NTSTATUS Status = RtlCreateUserThread(ProcessHandle,
		NULL, FALSE, 0, 0, 0, LoadLibraryAddress, DllFullPathBufferData,
		&ThreadHandle, NULL);
